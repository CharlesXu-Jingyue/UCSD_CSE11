...
class ExampleTweets {
    ...
    Tweet t1 = new TextTweet("no need for keystrokes or voices. I created this tweet just by thinking it. #helloworldbci", this.thomas, 11500);
    Tweet t2 = new ReplyTweet("Welcome to Twitter! What do you want the world to know about you, and what this has been like for you?", this.adam, 50, this.t1);
    Tweet t3 = new ReplyTweet("Dad says he's just a regular guy who was given an opportunity and that it's been quite an amazing journey", this.bianca, 133, this.t2);
    Tweet t4 = new ReplyTweet("Regular guys make the best heroes.", this.adam, 23, this.t3);
    Tweet t5 = new ReplyTweet("… and regular gals, of course. Bonus point when those guys/gals are forward-looking and can see the future coming.", this.adam, 6, this.t3);

    void testIsReplyTo(Tester t) {
        t.checkExpect(this.t1.isReplyTo(this.t2), false);
        t.checkExpect(this.t2.isReplyTo(this.t1), true);
        t.checkExpect(this.t5.isReplyTo(this.t3), true);
        t.checkExpect(this.t5.isReplyTo(this.t2), false);
        t.checkExpect(this.t4.isReplyTo(this.t3), true);
    }

    void testTotalLikes(Tester t) {
        t.checkExpect(this.t5.totalLikes(), 6 + 133 + 50 + 11500);
        t.checkExpect(this.t4.totalLikes(), 23 + 133 + 50 + 11500);
        t.checkExpect(this.t1.totalLikes(), 11500);
    }

    void testAllAuthors(Tester t) {
        t.checkExpect(this.t1.allAuthors(), "tomoxl");
        t.checkExpect(this.t2.allAuthors(), "AdamArthurMD;tomoxl");
        t.checkExpect(this.t3.allAuthors(), "bianca_okeefe;AdamArthurMD;tomoxl");
        t.checkExpect(this.t5.allAuthors(), "AdamArthurMD;bianca_okeefe;AdamArthurMD;tomoxl");
    }

    void testTextAppearsOnThread(Tester t) {
        t.checkExpect(this.t1.textAppearsOnThread("Thomas Oxley"), false);
        t.checkExpect(this.t1.textAppearsOnThread("tweet"), true);
        t.checkExpect(this.t2.textAppearsOnThread("tweet"), true);
        t.checkExpect(this.t2.textAppearsOnThread("Twitter"), true);
        t.checkExpect(this.t3.textAppearsOnThread("Daddy"), false);
        t.checkExpect(this.t4.textAppearsOnThread("hero"), true);
        t.checkExpect(this.t5.textAppearsOnThread("of course"), true);
    }

    void testCountPopularTweet(Tester t) {
        t.checkExpect(this.t1.countPopularTweet(10000), 1);
        t.checkExpect(this.t2.countPopularTweet(100), 1);
        t.checkExpect(this.t3.countPopularTweet(20), 3);
        t.checkExpect(this.t5.countPopularTweet(0), 4);
    }
}